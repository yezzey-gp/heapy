...
/*** behavior for ymdopen & _mdfd_getseg ***/
 ...
 * ymdnblocks() and related functionality henceforth - which currently is ok,
...
/*
 * GetYRelationPath - construct path to a relation's file
 *
 ...
 */
char *
GetYRelationPath(Oid dbNode, Oid spcNode, Oid relNode,
...
#define yrelpathbackend(rnode, backend, forknum) \
	GetYRelationPath((rnode).dbNode, (rnode).spcNode, (rnode).relNode, \
					backend, forknum)

/* First argument is a RelFileNode */
#define yrelpathperm(rnode, forknum) \
	yrelpathbackend(rnode, InvalidBackendId, forknum)

/* First argument is a RelFileNodeBackend */
#define yrelpath(rnode, forknum) \
	yrelpathbackend((rnode).node, (rnode).backend, forknum)


/* local routines */
static void ymdunlinkfork(RelFileNodeBackend rnode, ForkNumber forkNum,
						 bool isRedo);
static MdfdVec *ymdopen(SMgrRelation reln, ForkNumber forknum, int behavior);
static void yregister_dirty_segment(SMgrRelation reln, ForkNumber forknum,
								   MdfdVec *seg);
static void yregister_unlink_segment(RelFileNodeBackend rnode, ForkNumber forknum,
									BlockNumber segno);
static void yregister_forget_request(RelFileNodeBackend rnode, ForkNumber forknum,
									BlockNumber segno);
static void _yfdvec_resize(SMgrRelation reln,
						  ForkNumber forknum,
						  int nseg);
static char *_ymdfd_segpath(SMgrRelation reln, ForkNumber forknum,
						   BlockNumber segno);
static MdfdVec *_ymdfd_openseg(SMgrRelation reln, ForkNumber forkno,
							  BlockNumber segno, int oflags);
static MdfdVec *_ymdfd_getseg(SMgrRelation reln, ForkNumber forkno,
							 BlockNumber blkno, bool skipFsync, int behavior);
static BlockNumber _ymdnblocks(SMgrRelation reln, ForkNumber forknum,
							  MdfdVec *seg);
 ...
 */
bool
ymdexists(SMgrRelation reln, ForkNumber forkNum)
{
	/*
	 * Close it first, to ensure that we notice if the fork has been unlinked
	 * since we opened it.
	 */
	ymdclose(reln, forkNum);

	return (ymdopen(reln, forkNum, EXTENSION_RETURN_NULL) != NULL);
}
 ...
 */
void
ymdcreate(SMgrRelation reln, ForkNumber forkNum, bool isRedo)
{
	MdfdVec    *mdfd;
	char	   *path;
	File		fd;
	...
	path = yrelpath(reln->smgr_rnode, forkNum);
 ...
 */
void
ymdcreate_ao(RelFileNodeBackend rnode, int32 segmentFileNum, bool isRedo)
{
	char	   *path;
	char		buf[MAXPGPATH];
	File		fd;
	...
	if (fd < 0)
	{
		 ...
		 * already, even if isRedo is not set.	(See also ymdopen)
		 */
 ...
 */
void
ymdunlink(RelFileNodeBackend rnode, ForkNumber forkNum, bool isRedo)
{
	/* Now do the per-fork work */
	if (forkNum == InvalidForkNumber)
	{
		for (forkNum = 0; forkNum <= MAX_FORKNUM; forkNum++)
			ymdunlinkfork(rnode, forkNum, isRedo);
	}
	else
		ymdunlinkfork(rnode, forkNum, isRedo);
}
...
static void
ymdunlinkfork(RelFileNodeBackend rnode, ForkNumber forkNum, bool isRedo)
{
	char	   *path;
	int			ret;

	path = yrelpath(rnode, forkNum);
	...
	if (isRedo || forkNum != MAIN_FORKNUM || RelFileNodeBackendIsTemp(rnode))
	{
		if (!RelFileNodeBackendIsTemp(rnode))
		{
			...
			yregister_forget_request(rnode, forkNum, 0 /* first seg */ );
		}
		else
			ret = 0;
	...
	else
	{
		...
		yregister_unlink_segment(rnode, forkNum, 0 /* first seg */ );
	}
	...
	if (ret >= 0)
	{
		...
		for (segno = 1;; segno++)
		{
			...
			if (!RelFileNodeBackendIsTemp(rnode))
			{
				 ...
				 * try to unlink.
				 */
				yregister_forget_request(rnode, forkNum, segno);
			}
...
/*
 *	ymdextend() -- Add a block to the specified relation.
 *
 ...
 */
void
ymdextend(SMgrRelation reln, ForkNumber forknum, BlockNumber blocknum,
		 char *buffer, bool skipFsync)
{
	...
	Assert(blocknum >= ymdnblocks(reln, forknum));
#endif
		...
		ereport(ERROR,
				(errcode(ERRCODE_PROGRAM_LIMIT_EXCEEDED),
				 errmsg("cannot extend file \"%s\" beyond %u blocks",
						yrelpath(reln->smgr_rnode, forknum),
						InvalidBlockNumber)));

	v = _ymdfd_getseg(reln, forknum, blocknum, skipFsync, EXTENSION_CREATE);
	...
	if (!skipFsync && !SmgrIsTemp(reln))
		yregister_dirty_segment(reln, forknum, v);

	Assert(_ymdnblocks(reln, forknum, v) <= ((BlockNumber) RELSEG_SIZE));
}

/*
 *	ymdopen() -- Open the specified relation.
 *
 ...
 */
static MdfdVec *
ymdopen(SMgrRelation reln, ForkNumber forknum, int behavior)
{
	MdfdVec    *mdfd;
	char	   *path;
	File		fd;
	...
	Assert(_ymdnblocks(reln, forknum, mdfd) <= ((BlockNumber) RELSEG_SIZE));

	return mdfd;
}

/*
 *	ymdclose() -- Close the specified relation, if it isn't closed already.
 */
void
ymdclose(SMgrRelation reln, ForkNumber forknum)
{
	...
	while (nopensegs > 0)
	{
		...
		_yfdvec_resize(reln, forknum, nopensegs - 1);
		nopensegs--;
	}
}

/*
 *	ymdprefetch() -- Initiate asynchronous read of the specified block of a relation
 */
void
ymdprefetch(SMgrRelation reln, ForkNumber forknum, BlockNumber blocknum)
{
#ifdef USE_PREFETCH
	off_t		seekpos;
	MdfdVec    *v;

	v = _ymdfd_getseg(reln, forknum, blocknum, false, EXTENSION_FAIL);
...
/*
 * ymdwriteback() -- Tell the kernel to write pages back to storage.
 *
 * This accepts a range of blocks because flushing several pages at once is
 * considerably more efficient than doing so individually.
 */
void
ymdwriteback(SMgrRelation reln, ForkNumber forknum,
			BlockNumber blocknum, BlockNumber nblocks)
{
	...
	while (nblocks > 0)
	{
		...
		v = _ymdfd_getseg(reln, forknum, blocknum, true /* not used */ ,
 ...
 */
void
ymdread(SMgrRelation reln, ForkNumber forknum, BlockNumber blocknum,
	   char *buffer)
{
	off_t		seekpos;
	int			nbytes;
	...
	v = _ymdfd_getseg(reln, forknum, blocknum, false,
...
/*
 *	ymdwrite() -- Write the supplied block at the appropriate location.
 *
 ...
 */
void
ymdwrite(SMgrRelation reln, ForkNumber forknum, BlockNumber blocknum,
		char *buffer, bool skipFsync)
{
	off_t		seekpos;
	int			nbytes;
	...
	v = _ymdfd_getseg(reln, forknum, blocknum, skipFsync,
					 EXTENSION_FAIL | EXTENSION_CREATE_RECOVERY);
...
/*
 *	ymdnblocks() -- Get the number of blocks stored in a relation.
 *
 ...
 */
BlockNumber
ymdnblocks(SMgrRelation reln, ForkNumber forknum)
{
	MdfdVec    *v = ymdopen(reln, forknum, EXTENSION_FAIL);
	BlockNumber nblocks;
	BlockNumber segno = 0;

	/* ymdopen has opened the first segment */
	Assert(reln->md_num_open_segs[forknum] > 0);
	...
	for (;;)
	{
		nblocks = _ymdnblocks(reln, forknum, v);
		 ...
		 */
		v = _ymdfd_openseg(reln, forknum, segno, 0);
		if (v == NULL)
 ...
 */
void
ymdtruncate(SMgrRelation reln, ForkNumber forknum, BlockNumber nblocks)
{
	BlockNumber curnblk;
	...
	/*
	 * NOTE: ymdnblocks makes sure we have opened all active segments, so that
	 * truncation loop will get them all!
	 */
	curnblk = ymdnblocks(reln, forknum);
	if (nblocks > curnblk)
	{
	...
	while (curopensegs > 0)
	{
		...
		if (priorblocks > nblocks)
		{
			...
			FileClose(v->mdfd_vfd);
			_yfdvec_resize(reln, forknum, curopensegs - 1);
		}
 ...
 */
void
ymdimmedsync(SMgrRelation reln, ForkNumber forknum)
{
	int			segno;
 ...
 */
static void
yregister_dirty_segment(SMgrRelation reln, ForkNumber forknum, MdfdVec *seg)
{
	FileTag		tag;
...
/*
 * yregister_unlink_segment() -- Schedule a file to be deleted after next checkpoint
 */
static void
yregister_unlink_segment(RelFileNodeBackend rnode, ForkNumber forknum,
						BlockNumber segno)
{
	FileTag		tag;
...
/*
 * yregister_forget_request() -- forget any fsyncs for a relation fork's segment
 */
static void
yregister_forget_request(RelFileNodeBackend rnode, ForkNumber forknum,
						BlockNumber segno)
{
	FileTag		tag;
 ...
 */
static void
_yfdvec_resize(SMgrRelation reln,
			  ForkNumber forknum,
			  int nseg)
{
	if (nseg == 0)
 ...
 */
static char *
_ymdfd_segpath(SMgrRelation reln, ForkNumber forknum, BlockNumber segno)
{
	char	   *path,
			   *fullpath;
 ...
 */
static MdfdVec *
_ymdfd_openseg(SMgrRelation reln, ForkNumber forknum, BlockNumber segno,
			  int oflags)
{
	MdfdVec    *v;
	int			fd;
	...
	fullpath = _ymdfd_segpath(reln, forknum, segno);
	...
	if (segno <= reln->md_num_open_segs[forknum])
		_yfdvec_resize(reln, forknum, segno + 1);
...
/*
 *	_ymdfd_getseg() -- Find the segment of the relation holding the
 *		specified block.
 *
 ...
 */
static MdfdVec *
_ymdfd_getseg(SMgrRelation reln, ForkNumber forknum, BlockNumber blkno,
			 bool skipFsync, int behavior)
{
	...
	{
		v = ymdopen(reln, forknum, behavior);
		if (!v)
	...
	{
		BlockNumber nblocks = _ymdnblocks(reln, forknum, v);
		int			flags = 0;
				 ...
				 nblocks < ((BlockNumber) RELSEG_SIZE))
		{
			...
			{
				/*
				 * Some callers discern between reasons for _ymdfd_getseg()
			...
			ereport(ERROR,
					(errcode_for_file_access(),
					 errmsg("could not open file \"%s\" (target block %u): previous segment is only %u blocks",
							_ymdfd_segpath(reln, forknum, nextsegno),
							blkno, nblocks)));
		}

		v = _ymdfd_openseg(reln, forknum, nextsegno, flags);
 ...
 */
static BlockNumber
_ymdnblocks(SMgrRelation reln, ForkNumber forknum, MdfdVec *seg)
{
	off_t		len;
 ...
 */
int
ymdsyncfiletag(const FileTag *ftag, char *path)
{
	...
	else
	{
		...
		p = _ymdfd_segpath(reln, ftag->forknum, ftag->segno);
 ...
 */
int
ymdunlinkfiletag(const FileTag *ftag, char *path)
{
	char	   *p;

	/* Compute the path. */
	p = yrelpathperm(ftag->rnode, MAIN_FORKNUM);
	strlcpy(path, p, MAXPGPATH);
	pfree(p);
 ...
 */
bool
ymdfiletagmatches(const FileTag *ftag, const FileTag *candidate)
{
	/*
